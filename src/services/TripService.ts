...
export class TripService {
    ...
    async getZoneByPosition(lng: number, lat: number): Promise<Zone> {
        ...
        if (!zone) throw new NotFoundError('No se encontró una zona para la posición especificada, asegurate de que el destino se encuentre dentro de una zona válida');
        return zone;
    }
    ...
    async getEstimatedTripCost(data: GetEstimatedTripPricePayload): Promise<EstimatedTripCost> {
        ...
        const zone = await this.getZoneByPosition(data.destination_lng, data.destination_lat);
